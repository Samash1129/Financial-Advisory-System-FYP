...
def main(company, df, startdate, enddate):
    ...
    final_low_values = {'HistoricalVal': [{'Date': date, 'Value': value} for date, value in
    ...
    # output_folder = 'Low_Rates'
    # if not os.path.exists(output_folder):
    #     os.makedirs(output_folder)
    # output_path = os.path.join(
    #     output_folder, f"{company}_low_values.json")
    # with open(output_path, "w") as f:
    #     json.dump(final_low_values, f)
    
    # Save the data to a JSON file
    output_parent_folder = 'JSONs'
    output_folder = 'Low_Values'
    output_path = os.path.join(output_parent_folder, output_folder, f"{company}_low_values.json")
