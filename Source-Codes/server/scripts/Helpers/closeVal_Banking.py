...
def main(company, df, startdate, enddate):
    ...
    final_close_values = {'HistoricalVal': [{'Date': date, 'Value': value} for date, value in
    ...
    # output_folder = 'Close_Rates'
    # if not os.path.exists(output_folder):
    #     os.makedirs(output_folder)
    # output_path = os.path.join(
    #     output_folder, f"{company}_close_values.json")
    # with open(output_path, "w") as f:
    #     json.dump(final_close_values, f)
    
    # Save the data to a JSON file
    output_parent_folder = 'JSONs'
    output_folder = 'CLose_Values'
    output_path = os.path.join(output_parent_folder, output_folder, f"{company}_close_values.json")
